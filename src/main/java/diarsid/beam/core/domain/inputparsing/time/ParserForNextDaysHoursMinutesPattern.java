...
import static diarsid.beam.core.domain.inputparsing.time.TimeType.NEXT_DAYS_HOURS_AND_MINUTES;
...
class ParserForNextDaysHoursMinutesPattern implements TimePatternParser {
    ...
    public List<String> timePatterns() {
        return asList(
                "current or next month day and time:",
                "  01 21:35",
                "  01d 21h:35m",
                "  1 7:12");
    }
        ...
        if ( timePattern.trim().matches("\\d{1,2}d? \\d{1,2}h?:\\d{1,2}m?") ) {
            ...
            if ( 
                ...
                LocalDateTime time = now()
                        ...
                        .withSecond(0)
                        .withNano(0);
                return Optional.of(new Time(time, NEXT_DAYS_HOURS_AND_MINUTES));
            } else {
