...
public class SingleArgumentOperationRecognizer extends PrioritizedRecognizer {
    ...
    public SingleArgumentOperationRecognizer(CommandType type, ArgumentsMode argumentsMode) {
        this.type = type;
    ...
    private Command commandWithOrWithoutArgument(String arg) {
        if ( wordIsAcceptable(arg) ) {
            return new SingleStringCommand(this.type, arg);
        } else {
            return new SingleStringCommand(this.type);
        }
    }
