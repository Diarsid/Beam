...
public class OpenNotePathRecognizer extends NodeRecognizer {
    
    public OpenNotePathRecognizer() {
    }

    @Override
    public Command assess(Input input) {
        if ( input.hasNotRecognizedArgs() ) {
            return new SingleStringCommand(input.currentArg(), OPEN_PATH_IN_NOTE);
        } else {
